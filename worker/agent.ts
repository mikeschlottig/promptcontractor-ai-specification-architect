...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    try {
      ...
      if (method === 'POST' && url.pathname === '/draft/clear') {
        return this.handleClearDraft();
      }
      if (method === 'DELETE' && url.pathname === '/draft') {
        return this.handleClearDraft();
      }
  ...
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean }): Promise<Response> {
    ...
    try {
      ...
      if (stream) {
        ...
        (async () => {
          try {
            ...
            const response = await this.chatHandler!.processMessage(message, this.state.messages, this.state, (chunk: string) => {
              try {
                this.setState({
                  ...this.state,
      ...
      const response = await this.chatHandler.processMessage(message, this.state.messages, this.state);
      const assistantMessage = createMessage('assistant', response.content, response.toolCalls);
  ...
  /**
   * Clear the proposed contract draft (server-side) so it does not reappear after apply.
   */
  private handleClearDraft(): Response {
    this.setState({
      ...this.state,
      latestDraft: undefined,
    });
